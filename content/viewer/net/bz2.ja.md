...
head_title: ".NET BZ2 Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでBZ2を読み取り、レンダリング、表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のBZ2ファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでBZ2ファイルを読み取り、レンダリングし、表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でBZ2ファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにBZ2ファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでBZ2ファイルをロードします。
        *BZ2ファイルをPNG形式に変換するオプションを設定します。
...
demos:
    ...
    title: "BZ2ビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/bz2)Webサイトにアクセスして、BZ2ファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-bz2"
          title: "BZ2ファイル形式について"
          content: |
            BZ2は、主にUNIXまたはLinuxシステムで、BZIP2オープンソース圧縮方式を使用して生成された圧縮ファイルです。これは、単一のファイルの圧縮に使用され、複数のファイルのアーカイブを目的としたものではありません。これは、複数のファイルを1つのファイルにアーカイブするが、圧縮しない同じプラットフォームのTARファイル形式とは対照的です。 BZ2として圧縮されたファイルは、WinZipなどのアプリケーションで解凍できます。 BZIP2は、Run-Length Encoding（RLE）またはBurrows-Wheeler圧縮アルゴリズムを使用して、高レベルの圧縮を実現します。
