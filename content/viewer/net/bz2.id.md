...
head_title: ".NET BZ2 Viewer API - Baca, Lihat, Render di C# VB.NET"
head_description: "..NET document viewer API untuk membaca, merender, dan menampilkan BZ2 di semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core."

title: "BZ2 File Viewer untuk Aplikasi C# .NET"
description: "..NET document viewer API untuk membaca, merender, dan menampilkan file BZ2 dalam semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core. Lihat file yang dirender dengan format & tata letak yang benar dalam HTML5, PDF, atau sebagai gambar menggunakan beberapa baris kode."
...
steps:
    enable: true
    title_left: "Langkah-langkah untuk Melihat File BZ2 di C#"
    content_left: |
        [GroupDocs.Viewer](viewer/net/) memudahkan pengembang .NET untuk menambahkan fitur tampilan file BZ2 dalam aplikasi mereka dengan menerapkan beberapa langkah mudah.

        * Buat instance kelas Viewer dan muat file BZ2 dengan path lengkap.
        * Atur opsi untuk mengonversi file BZ2 ke format PNG.
    ...
    code: |
        ...
        using (Viewer viewer = new Viewer("sample.bz2"))
        {
        	// Setel opsi tampilan 
...
demos:
    enable: true
    title: "Demo Langsung Pemirsa BZ2"
    content: |
        Tampilkan file BZ2 sekarang juga dengan mengunjungi situs web [GroupDocs.Viewer](https://products.groupdocs.app/viewer/BZ2). Demo langsung memiliki manfaat sebagai berikut.
        
about_formats:
    enable: true
    format:
        - icon: "far fa-file-bz2"
          title: "Tentang Format Berkas BZ2"
          content: |
            BZ2 adalah file terkompresi yang dihasilkan menggunakan metode kompresi sumber terbuka BZIP2, sebagian besar pada sistem UNIX atau Linux. Ini digunakan untuk kompresi satu file dan tidak dimaksudkan untuk pengarsipan banyak file. Ini berbeda dengan format file TAR pada platform yang sama yang mengarsipkan banyak file ke dalam satu file tetapi tanpa kompresi. File yang dikompresi sebagai BZ2 dapat didekompresi dengan aplikasi seperti WinZip. BZIP2 menggunakan algoritma kompresi Run-Length Encoding (RLE) atau Burrows-Wheeler untuk mencapai tingkat kompresi yang tinggi.
