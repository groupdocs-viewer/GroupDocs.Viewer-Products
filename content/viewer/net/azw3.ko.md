...
############################# Head #############################
head_title: ".NET AZW3 뷰어 API - C# VB.NET에서 읽기, 보기, 렌더링"
head_description: ".NET 문서 뷰어 API는 모든 유형의 C#, ASP.NET, VB.NET 및 .NET Core 애플리케이션에서 AZW3을 읽고 렌더링하고 표시합니다."

############################# Header ############################
title: "AZW3 C# .NET 애플리케이션용 파일 뷰어" 
description: ".NET 문서 뷰어 API는 모든 유형의 C#, ASP.NET, VB.NET 및 .NET Core 애플리케이션에서 AZW3 파일을 읽고 렌더링하고 표시합니다. HTML5, PDF 또는 몇 줄의 코드를 사용하여 이미지로 진정한 형식 및 레이아웃으로 렌더링된 파일을 봅니다." 
...
steps:
    enable: true
    title_left: "C#에서 AZW3 파일을 렌더링하는 단계" 
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)를 사용하면 몇 단계 만에 AZW3을 HTML, JPEG, PNG 또는 PDF로 렌더링할 수 있습니다.

        * 원하는 패키지 관리자를 사용하여 [.NET용 GroupDocs.Viewer](https://www.nuget.org/packages/groupdocs.viewer)를 설치합니다. 
        * Viewer 클래스의 인스턴스를 생성하고 전체 경로로 AZW3 파일을 로드합니다. 
        * AZW3 파일을 HTML, PNG, JPEG 또는 PDF 형식으로 렌더링하는 옵션을 설정합니다. 
...
demos:
    enable: true
    title: "AZW3 뷰어 라이브 데모"
    content: |
        지금 바로 [GroupDocs.Viewer 온라인 앱](https://products.groupdocs.app/viewer/azw3) 웹사이트를 방문하여 AZW3 파일을 확인하세요.
    lang: "ko"
