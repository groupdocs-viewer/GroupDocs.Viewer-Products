...
head_title: ".NET PLT Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでPLTを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のPLTファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでPLTファイルを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でPLTファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにPLTファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでPLTファイルをロードします。
        *PLTファイルをPNG形式に変換するオプションを設定します。
...
demos:
    ...
    title: "PLTビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/plt)Webサイトにアクセスして、PLTファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-plt"
          title: "PLTファイル形式について"
          content: |
            PLTファイル形式は、Autodesk、Inc.によって導入されたベクターベースのプロッタファイルであり、特定のCADファイルの情報が含まれています。プロットの詳細には、制作の正確さと精度が必要です。PLTファイルを使用すると、すべての画像がドットではなく線を使用して印刷されるため、これが保証されます。この形式は、プロッタプリンタに情報を送信するために使用されるHPGLファイル形式に基づいています。 PLTファイルは、元のアプリケーション（AutodeskのAutoCADなど）で表示できますが、CorelDRAW Graphics Suiteなど、これらのファイルを操作するために使用できる他のアプリケーションもあります。多くのアプリケーションとAPIは、PLTファイル形式からDXF、PDF、JPEG、TIFF、PNG、BMP、CGM、SVG、PS、およびPCLへの変換をサポートしています。
