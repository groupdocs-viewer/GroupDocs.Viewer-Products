...
head_title: ".NET CGM Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでCGMを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のCGMファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでCGMファイルを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でCGMファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにCGMファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでCGMファイルをロードします。
        *CGMファイルをPNG形式に変換するオプションを設定します。
...
demos:
    ...
    title: "CGMビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/cgm)Webサイトにアクセスして、CGMファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-cgm"
          title: "CGMファイル形式について"
          content: |
            Computer Graphics Metafile（CGM）は、ベクターグラフィック（2D）、ラスターグラフィック、およびテキストを保存および交換するための、プラットフォームに依存しない無料の国際標準メタファイル形式です。 CGMは、オブジェクト指向のアプローチと、画像生成のための多くの機能の提供を使用します。 CGMは、これらのオブジェクト指向の特性を使用して、グラフィック要素を再成形して画像をレンダリングします。メタファイルには、他のファイルを定義するために必要な情報が含まれています。 CGMでは、テキストベースのソースファイルには、後でバイナリファイルにコンパイルできるすべてのグラフィック要素が含まれています。基本的に、CGMは、特定のプラットフォームやデバイスから独立して、2Dグラフィックデータ交換を容易にする方法です。
