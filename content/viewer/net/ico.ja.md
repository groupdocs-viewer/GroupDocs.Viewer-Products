...
head_title: ".NET ICO Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでICOを読み取り、レンダリング、表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のICOファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでICOファイルを読み取り、レンダリングし、表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でICOファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにICOファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでICOファイルをロードします。
        * COファイルをPNG形式に変換するオプションを設定します。
    ...
    code: |
        ```cs
        //ビューアをインスタンス化します
        using (Viewer viewer = new Viewer("sample.ico"))
        {
        	//表示オプションを設定します 
...
demos:
    ...
    title: "ICOビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/ico)Webサイトにアクセスして、今すぐICOファイルを表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-ico"
          title: "ICOファイル形式について"
          content: |
            ICO拡張子の付いたファイルは、MicrosoftWindowsでアプリケーションを表すためのアイコンとして使用される画像ファイルタイプです。これらは、ディスプレイの要件に合わせて、さまざまなサイズ、カラーサポート、および解像度で提供されます。 Microsoft Windowsでのもう1つの同様の画像ファイル形式は、カーソル表現用の.CURであり、画像ヘッダーにホットスポットを定義します。 MacOSでは、ICNSファイル形式はICOファイルと同じ目的を果たします。いくつかのオンラインWebサイトやアプリケーションは、そのようなファイルを作成し、BMP、PNGなどの他の画像形式をアイコンファイル形式に変換する機能を提供します。 ICOファイルの公式のIANA登録インターネットメディアタイプは、image/vnd.microsoft.iconです。
