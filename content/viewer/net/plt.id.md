...
head_title: ".NET PLT Viewer API - Baca, Lihat, Render di C# VB.NET"
head_description: ".NET document viewer API untuk membaca, merender, dan menampilkan PLT di semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core."

title: "PLT File Viewer untuk Aplikasi C# .NET"
description: ".NET document viewer API untuk membaca, merender, dan menampilkan file PLT dalam semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core. Lihat file yang dirender dengan format & tata letak yang benar dalam HTML5, PDF, atau sebagai gambar menggunakan beberapa baris kode."
...
steps:
    enable: true
    title_left: "Langkah-langkah untuk Melihat File PLT di C#"
    content_left: |
        [GroupDocs.Viewer](/viewer/net/) memudahkan pengembang .NET untuk menambahkan fitur tampilan file PLT dalam aplikasi mereka dengan menerapkan beberapa langkah mudah.

        * Buat instance kelas Viewer dan muat file PLT dengan path lengkap.
        * Atur opsi untuk mengonversi file PLT ke format PNG.
...
demos:
    enable: true
    title: "Demo Langsung Pemirsa PLT"
    content: |
        Tampilkan file PLT sekarang juga dengan mengunjungi situs web [GroupDocs.Viewer](https://products.groupdocs.app/viewer/PLT). Demo langsung memiliki manfaat sebagai berikut.
...
about_formats:
    ...
    format:
        - icon: "far fa-file-plt"
          ...
          content: |
            Format file PLT adalah file plotter berbasis vektor yang diperkenalkan oleh Autodesk, Inc. dan berisi informasi untuk file CAD tertentu. Detail plot membutuhkan akurasi dan presisi dalam produksi, dan penggunaan file PLT menjamin ini karena semua gambar dicetak menggunakan garis, bukan titik. Format ini didasarkan pada format file HPGL yang digunakan untuk mengirim informasi ke printer plotter. File PLT dapat dilihat dengan aplikasi aslinya yaitu Autodesk AutoCAD, tetapi ada juga aplikasi lain yang dapat digunakan untuk memanipulasi file ini seperti CorelDRAW Graphics Suite. Sejumlah aplikasi dan API mendukung konversi format file PLT ke DXF, PDF, JPEG, TIFF, PNG, BMP, CGM, SVG, PS dan PCL.
