...
head_title: ".NET PST Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでPSTを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のPSTファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでPSTファイルを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でPSTファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにPSTファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでPSTファイルをロードします。
        * STファイルをPNG形式に変換するオプションを設定します。
...
demos:
    ...
    title: "PSTビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/pst)Webサイトにアクセスして、PSTファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-pst"
          title: "PSTファイル形式について"
          content: |
            拡張子が.PSTのファイルは、さまざまなユーザー情報を格納するOutlook個人用ストレージファイル（個人用ストレージテーブルとも呼ばれます）を表します。ユーザー情報は、電子メール、カレンダーアイテム、メモ、連絡先、およびその他のいくつかのファイル形式を含むさまざまな種類のフォルダーに保存されます。 PSTファイルは、電子メールデータをオフラインでアーカイブするために使用されます。このデータは、後でさまざまなアプリケーションにロードして表示できます。
