...
head_title: ".NET C Viewer API - Baca, Lihat, Render di C# VB.NET"
head_description: ".NET document viewer API untuk membaca, merender, dan menampilkan C di semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core."

title: "C File Viewer untuk C# .NET Applications"
description: ".NET document viewer API untuk membaca, merender, dan menampilkan file C dalam semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core. Lihat file yang dirender dengan format & tata letak yang benar dalam HTML5, PDF, atau sebagai gambar menggunakan beberapa baris kode."
...
steps:
    enable: true
    title_left: "Langkah-langkah untuk Melihat File C di C#"
    content_left: |
        [GroupDocs.Viewer](/viewer/net/) memudahkan pengembang .NET untuk menambahkan fitur tampilan file C dalam aplikasi mereka dengan menerapkan beberapa langkah mudah.

        * Buat instance kelas Viewer dan muat file C dengan path lengkap.
        * Atur opsi untuk mengonversi file C ke format PNG.
    ...
    code: |
        ...
        using (Viewer viewer = new Viewer("sample.c"))
        {
        	// Setel opsi tampilan 
...
demos:
    enable: true
    title: "Demo Langsung Pemirsa C"
    content: |
        Tampilkan file C sekarang juga dengan mengunjungi situs web [GroupDocs.Viewer](https://products.groupdocs.app/viewer/C). Demo langsung memiliki manfaat sebagai berikut.
        
about_formats:
    enable: true
    format:
        - icon: "far fa-file-c"
          title: "Tentang Format Berkas C"
          content: |
            File dengan ekstensi .C adalah file kode sumber untuk menulis aplikasi perangkat lunak dalam bahasa pemrograman C. File C mencakup semua implementasi fungsionalitas aplikasi dalam bentuk kode sumber. Deklarasi kode sumber ditulis dalam file header yang disimpan dengan ekstensi .h. C++ adalah bentuk modern dari bahasa C dan digunakan untuk mengembangkan sebagian besar perangkat lunak saat ini. File C dapat dibuka dengan aplikasi seperti Notepad++, Eclipse IDE, Microsoft Visual Studio, dan Borland C++ Builder.
