...
head_title: ".NET Archive Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET Coreアプリケーションでアーカイブファイルを読み取り、レンダリング、表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のアーカイブファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET Coreアプリケーションでアーカイブファイルを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でアーカイブファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにアーカイブファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでZIPファイルをロードします。
        *ZIPファイルをPNG形式に変換するオプションを設定します。
...
demos:
    enable: true
    title: "アーカイブビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/zip)Webサイトにアクセスして、アーカイブファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-archive-o"
          title: "アーカイブファイル形式について"
          content: |
            ファイル圧縮は、1つ以上のファイルのサイズを縮小するプロセスです。不要なデータを削除することで、大きなファイルをはるかに小さなファイルに縮小します。圧縮されたファイルアーカイブにより、大きなファイルまたはファイルのグループの送信とバックアップが簡単になります。さらに、このようなファイルを使用すると、ダウンロードが速くなり、リムーバブルメディアに保存できるデータが増えます。さまざまな圧縮形式があります。以下では、最も一般的な圧縮ファイル形式とそれを使用する最も推奨されるソフトウェアについて説明します。
