...
head_title: ".NET RAR Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでRARを読み取り、レンダリング、表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のRARファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでRARファイルを読み取り、レンダリングし、表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でRARファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにRARファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでRARファイルをロードします。
        * ARファイルをPNG形式に変換するオプションを設定します。
...
demos:
    ...
    title: "RARビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/rar)Webサイトにアクセスして、RARファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-archive-o"
          title: "RARファイル形式について"
          content: |
            拡張子が.rarのファイルは、情報を圧縮形式または通常の形式で保存するために作成されるアーカイブファイルです。 Roshal ARchiveファイル形式の略であるRARは、ロシアのソフトウェアエンジニアであったEugeneRoshalによって1995年に作成された独自のファイル形式です。この形式は、さまざまな圧縮技術を含むさまざまな方法でファイルをアーカイブするために使用されます。 RARファイルを抽出するために、Windows、Linux、およびMacOSで使用できるアプリケーションソフトウェアがいくつかあります。 RARLabによるWinRARソフトウェアは、Microsoft Windowsプラットフォーム用のシェアウェアファイルアーカイブユーティリティ（40日間無料）です。ソフトウェアは、同じ作者であるEugene RoshalによってLinuxに（エクストラクタとしてのみ）移植されました。
