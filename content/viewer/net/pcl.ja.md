...
head_title: ".NET PCL Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでPCLを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のPCLファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでPCLファイルを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でPCLファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにPCLファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでPCLファイルをロードします。
        *PCLファイルをPNG形式に変換するオプションを設定します。
...
demos:
    ...
    title: "PCLビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/pcl)Webサイトにアクセスして、PCLファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-pcl"
          title: "PCLファイル形式について"
          content: |
            PCLは、Hewlett Packard（HP）によって導入されたページ記述言語であるPrinterCommandLanguageの略です。 HPは、さまざまな印刷デバイス間でプリンター機能を制御するための効率的な方法を提供するためにPCLを作成しました。このフォーマットは、もともとHPのドットマトリックスおよびインクジェットプリンター用に開発されましたが、時間の経過とともにさまざまなサーマル、マトリックス、およびページプリンターの一部になりました。フォーマットはいくつかの異なる改訂を受け、その結果、各バージョンがプリンター制御機能に関する時間の要求を満たすように拡張された異なるバージョンになりました。今日、PCLは、最新のプリンター市場で最も広く普及しているプリンター言語です。
