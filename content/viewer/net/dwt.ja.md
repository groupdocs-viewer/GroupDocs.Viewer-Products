...
head_title: ".NET DWT Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでDWTを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のDWTファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでDWTファイルを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でDWTファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにDWTファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでDWTファイルをロードします。
        *DWTファイルをPNG形式に変換するオプションを設定します。
...
demos:
    ...
    title: "DWTビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/dwt)Webサイトにアクセスして、今すぐDWTファイルを表示してください。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-dwt"
          title: "DWTファイル形式について"
          content: |
            DWTファイルは、DWGファイルとして保存できる図面を作成するためのスターターとして使用されるAutoCAD図面テンプレートファイルです。このようなテンプレートファイルは、単位タイプ、必要な精度、タイトルブロック、レイヤー名、線種、および適切な図面ファイルに横方向に変換するための他の同様の情報などの初期設定を提供します。 AutoCADとCoreCADの両方を使用して、テンプレートファイルを読み取り、さらに利用することができます。簡単に言うと、DWTは、MSWord DOTファイルがDOCファイルに対して行われるのと同じように、事前定義された読み取り専用属性を持つDWGです。
