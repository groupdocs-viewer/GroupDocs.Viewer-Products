...
head_title: ".NET PSB Viewer API-C＃VB.NETでの読み取り、表示、レンダリング"
head_description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでPSBを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI."

title: "C＃.NETアプリケーション用のPSBファイルビューア"
description: ".あらゆるタイプのC＃、ASP.NET、VB.NET、および.NET CoreアプリケーションでPSBファイルを読み取り、レンダリング、および表示するためのNETドキュメントビューアAPI。レンダリングされたファイルを、HTML5、PDFで、またはコードの数行を使用して画像として、実際のフォーマットとレイアウトで表示します."
...
steps:
    enable: true
    title_left: "C＃でPSBファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーションにPSBファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでPSBファイルをロードします。
        * SBファイルをPNG形式に変換するオプションを設定します。
...
demos:
    ...
    title: "PSBビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/psb)Webサイトにアクセスして、PSBファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-image-o"
          title: "PSBファイル形式について"
          content: |
            Adobe Photoshopは、ファイルを2つの形式で保存します。サイズが30,000x30,000ピクセルのファイルはPSD拡張子で保存され、PSDより最大300,000x300,000ピクセルのファイルは「PhotoshopBig」と呼ばれるPSB拡張子で保存されます。より具体的には、PSBファイルは最大4 EB（42億GB以上）で、高さと幅が最大300,000ピクセルの画像を使用できます。一方、PSDは最大2 GBで、画像のサイズは30,000ピクセルです。 PSBは、フォトショップの大フォーマットサイズとも呼ばれ、レイヤー、エフェクト、フィルターなどのすべてのフォトショップ機能をサポートします。フォトショップは、PSBファイルをPSD、JPG、PNG、EPS、GIF、その他のフォーマットに変換することもできます。 Photoshopの設定ダイアログボックスのファイル処理ペインの機能を有効にすると、Photoshopの大きなドキュメント形式を使用できるようになります。
