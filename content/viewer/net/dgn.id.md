...
head_title: ".NET DGN Viewer API - Baca, Lihat, Render di C# VB.NET"
head_description: "..NET document viewer API untuk membaca, merender, dan menampilkan DGN di semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core."

title: "DGN File Viewer untuk Aplikasi C# .NET"
description: "..NET document viewer API untuk membaca, merender, dan menampilkan file DGN dalam semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core. Lihat file yang dirender dengan format & tata letak yang benar dalam HTML5, PDF, atau sebagai gambar menggunakan beberapa baris kode."
...
steps:
    enable: true
    title_left: "Langkah-langkah untuk Melihat File DGN di C#"
    content_left: |
        [GroupDocs.Viewer](viewer/net/) memudahkan pengembang .NET untuk menambahkan fitur tampilan file DGN dalam aplikasi mereka dengan menerapkan beberapa langkah mudah.

        * Buat instance kelas Viewer dan muat file DGN dengan path lengkap.
        * Atur opsi untuk mengonversi file DGN ke format PNG.
    ...
    code: |
        ...
        using (Viewer viewer = new Viewer("sample.dgn"))
        {
        	// Setel opsi tampilan 
...
demos:
    enable: true
    title: "Demo Langsung Pemirsa DGN"
    content: |
        Tampilkan file DGN sekarang juga dengan mengunjungi situs web [GroupDocs.Viewer](https://products.groupdocs.app/viewer/DGN). Demo langsung memiliki manfaat sebagai berikut.
...
about_formats:
    ...
    format:
        - icon: "far fa-file-dgn"
          ...
          content: |
            DGN, Desain, file adalah gambar yang dibuat oleh dan didukung oleh aplikasi CAD seperti MicroStation dan Intergraph Interactive Graphics Design System. Ini digunakan untuk membuat dan menyimpan desain untuk proyek konstruksi seperti jalan raya, jembatan, dan bangunan. Formatnya mirip dengan format file DWG Autodesk dan dianggap sebagai pesaingnya. File DNG dapat disimpan sebagai Format File Standar Intergraph atau V8 DGN. DGN dapat dikonversi ke beberapa format lain seperti DWG, BMP, JPEG, PDF, GIF, dan lainnya. Itu dapat dibuka dengan Autodesk AutoCAD, Bentley View dan Bentley Systems MicroStation selain aplikasi perangkat lunak lain seperti versi Corel PaintShop Photo Pro dan IMSI TurboCAD Deluxe.
