...
head_title: ".NET TAR Viewer API - Baca, Lihat, Render di C# VB.NET"
head_description: "..NET document viewer API untuk membaca, merender, dan menampilkan TAR di semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core."

title: "TAR File Viewer untuk Aplikasi C# .NET"
description: "..NET document viewer API untuk membaca, merender, dan menampilkan file TAR dalam semua jenis aplikasi C#, ASP.NET, VB.NET & .NET Core. Lihat file yang dirender dengan format & tata letak yang benar dalam HTML5, PDF, atau sebagai gambar menggunakan beberapa baris kode."
...
steps:
    enable: true
    title_left: "Langkah-langkah untuk Melihat File TAR di C#"
    content_left: |
        [GroupDocs.Viewer](viewer/net/) memudahkan pengembang .NET untuk menambahkan fitur tampilan file TAR dalam aplikasi mereka dengan menerapkan beberapa langkah mudah.

        * Buat instance kelas Viewer dan muat file TAR dengan path lengkap.
        * Atur opsi untuk mengonversi file TAR ke format PNG.
    ...
    code: |
        ...
        using (Viewer viewer = new Viewer("sample.tar"))
        {
        	// Setel opsi tampilan 
...
demos:
    enable: true
    title: "Demo Langsung Pemirsa TAR"
    content: |
        Tampilkan file TAR sekarang juga dengan mengunjungi situs web [GroupDocs.Viewer](https://products.groupdocs.app/viewer/TAR). Demo langsung memiliki manfaat sebagai berikut.
        
about_formats:
    enable: true
    format:
        - icon: "far fa-file-tar"
          title: "Tentang Format Berkas TAR"
          content: |
            File dengan ekstensi TAR adalah arsip yang dibuat dengan utilitas berbasis Unix untuk mengumpulkan satu atau lebih file. Beberapa file disimpan dalam format yang tidak terkompresi dengan dukungan penambahan file serta folder ke arsip. Utilitas TAR di Unix berbasis Perintah, tetapi file yang dibuat didukung oleh sebagian besar sistem pengarsipan file di hampir semua sistem operasi. Ini pertama kali dibuat pada tahun 1979 oleh AT&T Bell Laboratories dan versi selanjutnya diterbitkan dengan berlalunya waktu.
