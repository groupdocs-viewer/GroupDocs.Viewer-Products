...
############################# Head #############################
head_title: ".NET JSON 查看器 API - 在 C# VB.NET 中讀取、查看、渲染"
head_description: ".NET 文檔查看器 API，用於在任何類型的 C#、ASP.NET、VB.NET 和 .NET Core 應用程序中讀取、渲染和顯示 JSON。"

############################# Header ############################
title: "用於 C# .NET 應用程序的 JSON 文件查看器" 
description: ".NET 文檔查看器 API，用於在任何類型的 C#、ASP.NET、VB.NET 和 .NET Core 應用程序中讀取、渲染和顯示 JSON 文件。使用 HTML5、PDF 或使用幾行代碼以圖像形式查看具有真實格式和佈局的渲染文件。" 
...
steps:
    enable: true
    title_left: "在 C# 中渲染 JSON 文件的步驟" 
    content_left: |
        使用 [GroupDocs.Viewer](https://products.groupdocs.com/viewer/net/)，您只需幾個步驟即可將 JSON 呈現為 HTML、JPEG、PNG 或 PDF。

        * 使用您最喜歡的包管理器安裝 [GroupDocs.Viewer for .NET](https://www.nuget.org/packages/groupdocs.viewer)。 
        * 創建 Viewer 類的實例並加載具有完整路徑的 JSON 文件。 
        * 設置選項以將 JSON 文件呈現為 HTML、PNG、JPEG 或 PDF 格式。 
        * 渲染文件並檢查當前目錄中的輸出。 
...
demos:
    ...
    title: "JSON查看器現場演示"
    content: |
        立即訪問 [GroupDocs.Viewer Online Apps](https://products.groupdocs.app/viewer/json) 網站查看 JSON 文件。
    lang: "zh-hant"
