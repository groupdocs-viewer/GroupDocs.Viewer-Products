...
head_title: "Java OneNoteViewerAPI-JavaアプリでのOneNoteのレンダリングと表示"
head_description: "Java、J2EE、J2SEアプリケーションでOneNoteファイルを表示します。ドキュメント表示オプションを管理するための高度な機能を備えた、HTML、PDF、または画像モードでの150以上のドキュメントおよび画像ファイル形式の表示をサポート."

title: "JavaでOneNoteファイルをレンダリングおよび表示する"
description: "Java、J2EE、およびJ2SEベースのアプリケーション用のネイティブで高性能なOneNoteファイルビューアAPI。出力ドキュメント形式の外観をカスタマイズするためのさまざまな追加機能をサポートします。"
...
steps:
    enable: true
    title_left: "JavaでOneNoteファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/java/)を使用すると、Java開発者は、数行のコードを使用して、アプリケーションにOneNoteファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでONEファイルをロードします。
        *1つのファイルをPNG形式に変換するように表示オプションを設定します。
...
demos:
    enable: true
    title: "OneNoteViewerのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/)Webサイトにアクセスして、OneNoteファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-sticky-note-o"
          ...
          content: |
            メモを取るプログラムファイルには、メモを保存するためのセクションとページが含まれています。メモ文書は、テキスト文書のように単純なものから、デジタル画像、オーディオ/ビデオクリップ、および手描きのスケッチ図で構成されるより詳細なものにすることができます。一般的なOneNoteファイル拡張子とそれに関連するファイル形式には、.ONEとその目次ファイル.ONETOC2が含まれます。
