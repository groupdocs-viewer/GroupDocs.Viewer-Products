...
draft: false
############################# Head ############################
head_title: "Java GZIP Viewer API — рендеринг и отображение GZIP в приложениях Java"
head_description: "Просмотр файлов GZIP в приложениях Java, J2EE, J2SE. Поддерживает просмотр более 150 форматов файлов документов и изображений в формате HTML, PDF или изображения с расширенными функциями для управления параметрами просмотра документов."
############################# Header ############################
title: "Рендеринг и просмотр файла GZIP в Java"
description: "Собственный и высокопроизводительный API-интерфейс просмотра файлов GZIP для приложений на основе Java, J2EE и J2SE, поддерживающий широкий спектр дополнительных функций для настройки внешнего вида формата выходного документа."
bg_overlay: false
button:
    enable: true
...
steps:
    enable: true
    title_left: "Шаги для просмотра файла GZIP в Java"
    content_left: |
        [GroupDocs.Viewer](/ru/viewer/java/) позволяет разработчикам Java легко добавлять функцию просмотра файлов GZIP в свои приложения с помощью нескольких строк кода.
        * Создайте экземпляр класса Viewer и загрузите файл GZIP с полным путем.
        * Установите параметры просмотра для преобразования файла GZIP в формат PNG.
    ...
    code: |
        ...
        try (Viewer viewer = new Viewer("sample.gzip"))
        {
...
demos:
    enable: true
    title: "Демонстрации GZIP Viewer в реальном времени"
    content: |
        Отобразите файл GZIP прямо сейчас, посетив веб-сайт [GroupDocs.Viewer Живые волнения](https://products.groupdocs.app/viewer/).
############################# About Formats ############################
about_formats:
    enable: true
    format:
        ...
        - icon: "far fa-file-archive-o"
          title: "О формате файла GZ"
          content: |
            Файлы с расширением .gz представляют собой сжатые файлы, созданные с помощью приложения сжатия gzip. Он может содержать несколько сжатых файлов и обычно используется в системах UNIX и Linux. GZIP был представлен как бесплатная утилита для замены программы Compress, используемой в системах Unix. Такие файлы можно открывать и извлекать с помощью нескольких приложений, таких как WinZip, которые доступны как в Windows, так и в MacOS. Хотя этот формат похож на ZIP-сжатие при архивировании, он отличается с точки зрения сжатия, применяемого к архиву, а не к отдельному файлу.
