...
head_title: "Java SXCViewerAPI-JavaアプリでSXCをレンダリングおよび表示"
head_description: "Java、J2EE、J2SEアプリケーションでSXCファイルを表示します。ドキュメント表示オプションを管理するための高度な機能を備えた、HTML、PDF、または画像モードでの150以上のドキュメントおよび画像ファイル形式の表示をサポート."

title: "JavaでSXCファイルをレンダリングおよび表示"
description: "Java、J2EE、およびJ2SEベースのアプリケーション用のネイティブで高性能なSXCファイルビューアAPI。出力ドキュメント形式の外観をカスタマイズするためのさまざまな追加機能をサポートします。"
...
steps:
    enable: true
    title_left: "JavaでSXCファイルを表示する手順"
    content_left: |
        [GroupDocs.Viewer](https://products.groupdocs.com/viewer/java/)を使用すると、Java開発者は、数行のコードを使用して、アプリケーションにSXCファイル表示機能を簡単に追加できます。

        * Viewerクラスのインスタンスを作成し、フルパスでSXCファイルをロードします。
        *SXCファイルをPNG形式に変換するための表示オプションを設定します。
...
demos:
    ...
    title: "SXCビューアのライブデモ"
    content: |
        [GroupDocs.Viewer Live Demos](https://products.groupdocs.app/viewer/)Webサイトにアクセスして、SXCファイルを今すぐ表示します。  
        ライブデモには次の利点があります
...
about_formats:
    ...
    format:
        - icon: "far fa-file-sxc"
          title: "SXCファイル形式について"
          content: |
            ファイル形式SXC（Sun XML Calc）は、OpenOffice.orgというオフィススイートに属しています。この形式は、XMLベースのスプレッドシートファイル形式であるため、通常、ユーザーのスプレッドシートのニーズに対応します。 SXC形式は、数式、関数、マクロ、グラフをDataPilotとともにサポートします。これは、インポートされた生データの要約を自動的に個別化して提供するため、すばらしい機能です。このソフトウェアで作成されたファイルは、拡張子.sxcで保存されます。
